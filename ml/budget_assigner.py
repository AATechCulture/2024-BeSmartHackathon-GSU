...
with open('../backend/bank_data.json', 'r') as f:
    bank_data = json.load(f)

# Load user data from user_details.json
with open('../backend/user_details.json', 'r') as f:
    user_details = json.load(f)

def generate_user_data(iin):
    # Select a user to generate a financial plan
    user_id = iin  # Replace with any valid user ID from the data
    ...
    return monthly_spending, income, target_allocation

# Generate budget recommendations based on spending patterns and target allocation
def generate_budget_recommendations(spending_data, income, target_allocation):
    recommendations = []
